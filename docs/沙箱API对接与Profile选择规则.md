...
Skill 的 `SKILL.md`、`references/`、`assets/` 是上下文资源；`scripts/` 是可执行资源。执行脚本时必须走 ToolGateway、Approval、SandboxRunner，不允许模型或 Skill loader 直接在宿主机上运行脚本。

默认规则：

| Skill 来源 | 运行期脚本 | 依赖构建 | 网络 |
| --- | --- | --- | --- |
| 内置系统 Skill | 优先 ToolGateway/Trusted Runtime；纯通用脚本可走 `tool-basic` 或 `skill-polyglot-basic`，专门工作负载按第 5 节选择 | 构建期固定 | 默认关闭 |
| 项目/用户 Skill | 纯通用脚本默认 `skill-polyglot-basic`；Office/PDF/OCR/Browser 按工作负载选择 | `skill-build-polyglot` | 运行期默认关闭，构建期 allowlist |
| 第三方/Marketplace Skill | 纯通用脚本默认 `skill-polyglot-basic`，高风险可升级高隔离；专门工作负载按声明和扫描结果选择 | 必须注册期构建，不在对话运行期临时安装 | 按声明和策略裁决，默认关闭 |
...
- 对话运行中不应临时 `pip install` / `npm install`，除非用户明确授权且策略允许，并进入审计。
...
执行器必须把这些目录注入环境变量。Agent 生成代码、内置 Skills 和外部脚本都应优先使用 `INPUT_DIR`、`OUTPUT_DIR`、`TMPDIR`，而不是硬编码 `/workspace`、`/tmp`、用户 HOME、Windows 盘符路径或项目根目录。
...
-> Analyzer Registry
   -> shell_text
   -> python_source
   -> future: js/ts, powershell, shell AST, skill manifest
```
...
6. `execution/pathcontract` 已实现可插拔 PathValidator/Analyzer Registry，默认包含 `shell_text` 和 `python_source` 分析器；`execution/service.Runner` 支持产品侧注入自定义 validator。

仍需继续：

1. `FileSystemClient` 的 workspace 文件 CRUD 尚未接通，因为 genesis-sandbox OpenAPI 当前未暴露稳定 workspace 文件端点；已接通 artifact 下载/本地落盘主路径。
2. Enterprise bootstrap 仍需注入远程 sandbox backend、租户/用户/RBAC/credential/audit；这些不应放入通用 adapter。
3. Skill 脚本还需要专门 runner/service，以便在进入 `run_command` 前根据 manifest、文件类型、依赖扫描选择 Office/OCR/Browser profile。
4. Office/Browser 后续应沉淀能力专属 service，避免长期依赖模型临时拼 shell 命令。
5. 启动或健康检查应调用 `GET /v1/runtime-profiles`，发现目标 profile 不可用时按 `SandboxRequired` / `SandboxOptional` 语义处理。
