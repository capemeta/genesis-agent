...
Skill 的 `SKILL.md`、`references/`、`assets/` 是上下文资源；`scripts/` 是可执行资源。执行脚本时必须走 `run_skill_command` → SkillScriptService（Materialize → Stage → Profile → ExecutionRunner/SandboxSession），不允许模型或 Skill loader 直接在宿主机上按 embed 内部路径运行脚本，也不应默认用 `run_command` 拼 `python scripts/...`。

默认规则：

| Skill 来源 | 运行期脚本 | 依赖构建 | 网络 |
| --- | --- | --- | --- |
| 内置系统 Skill | 优先 `run_skill_command`；Office/PDF 按第 5 节选 `office-basic`/`office-ocr`；纯通用脚本可走 `skill-polyglot-basic` | 构建期固定 | 默认关闭 |
...
- 对话运行中**默认禁止**临时 `pip install` / `npm install`。例外：经专用工具 `install_skill_dependencies`（或等价授权通道）审批后，使用 `skill-build-polyglot` + 网络 allowlist 安装，并写入审计；装完由 Agent 再调 `run_skill_command`。完整协议见 `docs/Skill三模式执行与依赖闭环设计.md`。
...
- 上层 Skill Script Service 对所有 `Result.Artifacts` 做统一归一化：只要存在 `local_path`，就必须以安全 basename 复制或映射到本次 Run 的 `OUTPUT_DIR`，并让返回给模型的 `artifacts[].path` 指向该目录内路径。这样后续步骤可直接使用 `$OUTPUT_DIR/<name>` 或 `artifacts[].path`，不需要额外一轮工具调用去猜测 `.genesis/artifacts` 中间路径。
