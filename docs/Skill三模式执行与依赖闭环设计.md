...
- 不把 `Skill()` 工具变成脚本执行器；脚本仍走 `run_skill_script`。
...
| I2 | 脚本入口统一为 `run_skill_script`（ResourceID），三模式只换 backend 映射 |
...
其次：对话期「结构化缺依赖 → 显式 install 通道 → Agent 再调 run_skill_script」
...
```text
run_skill_script(skill, script ResourceID, args[], inputs[])
  → SkillScriptService
...
`run_skill_script` 在 `ok=false` 时仍返回 JSON 正文；ReAct **不得**丢弃 stdout。
...
| 与业务脚本隔离 | 安装成功 ≠ 业务成功；必须由 Agent（或 opt-in 自动重试）再调 `run_skill_script` |
...
- **过渡**：若未上专用工具，office Skill 的 `allowed-tools` 可临时包含 `run_command`，但 SKILL.md 必须写明「仅允许 install 类命令；业务脚本仍用 `run_skill_script`」。  
...
```text
1. 若 run_skill_script 返回 failure_kind=dependency_missing 且 retryable=true：
   - 调用 install_skill_dependencies（或策略允许的等价安装命令）
   - 安装成功后，用相同 skill/script/args/inputs 再调用一次 run_skill_script
   - 不要在未安装成功时用同一参数死循环
...
| 规则 | 说明 |
| --- | --- |
| Meta 工具始终可见 | `Skill` / `list_skill_resources` / `read_skill_resource` / `run_skill_script` / **`install_skill_dependencies`** |
| 业务 Skill 可不列 `run_command` | 安装走专用工具，避免滥用 shell |
| SKILL.md 硬约束改写 | 「业务脚本必须用 run_skill_script；缺依赖时用 install_skill_dependencies，禁止用 write_file 假交付」 |
...
- [ ] 经审批安装后，第二次 `run_skill_script` 成功产出合法 `.pptx`（门禁通过）— 人工 E2E
...
1. **三模式**：统一 `run_skill_script` + Execution Workspace；产品注入 backend；降级策略显式。  
...
| P1 | `D:\workspace\go\go-project\Kode-CLI\packages\tools\src\tools\interaction\SkillTool\SkillTool.tsx` | Skill 只注入、不执行脚本 | 保持 `Skill` ≠ `run_skill_script` |
...
- `internal/capabilities/skill/tool/run_skill_script/tool.go`
...
| Skill 脚本走任意 Bash 拼路径 | 破坏 ResourceID / materialize / 门禁 | 只允许 `run_skill_script` |
