...
| `block_type` | string | 见 4.6.4 Block 类型全表 |
...
| `widget/a2ui` | A2UI 富 UI 组件（见 4.6.8） |
...
run.completed
  { output_block_index: 12,
    usage: { input_tokens: 1200, output_tokens: 800, thinking_tokens: 200 },
    duration_ms: 3400 }
  说明：Run 正常完成，output_block_index 指向 final_answer block
...
error.tool
  { error_code: "TOOL_EXEC_FAILED"|"TOOL_TIMEOUT"|"SANDBOX_ERROR",
    ...
    will_retry: false,
    will_replan: true }
  说明：工具失败，LLM 将重新规划，Run 继续；前端可标注该工具步骤失败
...
```text
ping
  { ts: "2026-06-28T12:00:30Z" }
  说明：保活心跳，每 15 秒发送一次，客户端无需响应（但可重置超时计时器）
...
SSE 的 `id:` 字段设为 `seq`，客户端断连后携带 `Last-Event-ID` 请求头重连：
