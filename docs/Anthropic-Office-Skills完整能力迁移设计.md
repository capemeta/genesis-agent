...
> 状态：Phase 1（office-ppt）实施中  
> 日期：2026-07-09（决策更新 2026-07-10）  
> 文档名建议简称：`Office Skills 完整能力迁移`  
> 源仓库：`D:\workspace\go\go-project\anthropics-skills\skills\{pptx,docx,xlsx,pdf}`  
> 目标：`internal/capabilities/skill/adapter/embedded/skills/{office-ppt,office-word,office-excel,pdf-review}`  
...
| `pdf`           | `pdf-review`           | 保持现名；能力升到 Anthropic `pdf` 完整度        |
...
| Excel 生成/模型规范   | SKILL 内完整财务/格式约定 + openpyxl 流程                   | inspect + recalc                   | **大**（知识+脚本）        |
...
| 共享  | `scripts/office/**`                                           | 同 pptx                     |
| 已有  | `inspect_docx.py`、`convert_docx_to_pdf.py`、`path_contract.py` | 保留并对齐                      |


### 4.3 `xlsx` → `office-excel`


| 类型  | 源路径                                  | 迁移动作                                                             |
| --- | ------------------------------------ | ---------------------------------------------------------------- |
| 主文档 | `SKILL.md`（财务模型/格式/公式约定）             | 合并进 `office-excel/SKILL.md` + 可拆 `references/financial-model.md` |
| 脚本  | `recalc.py`                          | 与现有 `recalc_xlsx.py` **合并对齐**，保留 Genesis 命名或提供别名                 |
| 共享  | `scripts/office/**`                  | 同 pptx（xlsx 侧也带了完整 office 树）                                     |
| 已有  | `inspect_xlsx.py`、`path_contract.py` | 保留                                                               |
...
| 类型  | 源路径                                                                                                                                                                                         | 迁移动作                                                      |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------- |
| 主文档 | `SKILL.md`、`forms.md`、`reference.md`                                                                                                                                                        | → SKILL + `references/forms.md`、`references/reference.md` |
| 脚本  | `convert_pdf_to_images.py`、`check_bounding_boxes.py`、`create_validation_image.py`、`extract_form_*`、`check_fillable_fields.py`、`fill_fillable_fields.py`、`fill_pdf_form_with_annotations.py` | → `scripts/`，统一 path_contract                             |
| 已有  | `inspect_pdf.py`、`extract_pdf_text.py`、`list_pdf_form_fields.py`、`render_pdf_pages.py`                                                                                                      | 保留；与 Anthropic 脚本功能重叠处做「保留 Genesis JSON 契约 / 吸收对方能力」      |
...
embedded/skills/
  ...
  office-word/
  office-excel/
  pdf-review/         # 一般不依赖 OOXML office/，独立
```
...
1. 迁入 forms/reference 与表单/合并/拆分/渲染脚本
2. 与现有 inspect/extract/render 去重，统一 JSON 契约
3. OCR 路径：inspect 推荐 `office-ocr` + Runner 自动升级（可与 Phase 1 并行开小项）

### Phase 5 — 收敛与产品化

1. [x] `_office_common` 去重（方案 A）
2. [x] Enterprise bootstrap 接线（embed Skills + SharedScriptsFS；headless Ask 自动批准；远程 sandbox/RBAC 仍待）
3. [ ] 更新 `docs/Office能力与Skills设计.md` 实现状态表
