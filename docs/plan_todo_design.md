...
7. 提醒不可复述原则 (Anti-Leakage Restriction):
   如果是执行过程中，系统向您发出了“【系统提醒】当前任务进度跟进...”的被动待办提示，请立即在您内部调整任务调度，但您**绝不能在回复给用户的最终文本中直接复述、背诵或提及任何关于系统提醒字样或该提醒的原始 Prompt 文本**。保持人机交互的纯净与高级感。
```

---

## 12. 审计结论：Plan & TodoList 设计与实现对齐审计报告 (Code-Doc Alignment)

我们通过映射 `docs/plan_todo_design.md` 中定义的各项设计重点，并审查实际代码，对其实现进度和一致性进行了评估：

### 12.1 对齐概览表

| 需求/设计要点 | 设计文档位置 | 代码实现位置 (Evidence) | 状态 | 偏差与说明 |
| :--- | :--- | :--- | :---: | :--- |
| **唯一进行中约束** | § 2.1 | [service.go:L127-136](file:///d:/workspace/go/genesis-agent/internal/capabilities/plan/service/service.go#L127-L136)<br>[service.go:L307-313](file:///d:/workspace/go/genesis-agent/internal/capabilities/plan/service/service.go#L307-L313) | `Implemented` | 对齐。在 `UpdatePlan` 和 `UpdateStepStatus` 中强制限制同时进行中的步骤数 <= 1。 |
| **被动提醒机制** | § 2.1 | [service.go:L352-399](file:///d:/workspace/go/genesis-agent/internal/capabilities/plan/service/service.go#L352-L399)<br>[builder.go:L185-207](file:///d:/workspace/go/genesis-agent/internal/bootstrap/builder.go#L185-L207) | `Implemented` | 对齐。系统通过 `GeneratePromptReminder` 每隔配置的 `remindIntervalSteps` 产生进度提醒，并在 ReAct Loop 启动时通过 `promptbuilder.ContextInjectorFunc` 进行注入。 |
| **已完成任务过滤归档** | § 2.1, 8.1 | [todo_read.go:L56-107](file:///d:/workspace/go/genesis-agent/internal/capabilities/tool/adapter/builtin/todo_read.go#L56-L107) | `Implemented` | 对齐。`todo_read` 工具只向 LLM 下发未完成步骤和最后 3 项已完成的参考步骤，并且智能保留具有未完成子孙的已完成父步骤。 |
| **稳定 ID 与模糊对齐** | § 6.1 | [service.go:L149-180](file:///d:/workspace/go/genesis-agent/internal/capabilities/plan/service/service.go#L149-L180) | `Implemented` | 对齐。结合 `MD5` 稳定 ID 与 `Levenshtein` 编辑距离对齐相似度大于 80% 的步骤 Title，保证了重构时 ID 稳定性。 |
| **人机协同合并** | § 6.1 | [service.go:L91-119](file:///d:/workspace/go/genesis-agent/internal/capabilities/plan/service/service.go#L91-L119) | `Implemented` | 对齐。合并逻辑在 Agent 全量提交时，优先保留用户对旧步骤的高权威标记（如 `completed` 状态）。 |
| **重大结构重构判定与审批** | § 6.1 | [service.go:L222-257](file:///d:/workspace/go/genesis-agent/internal/capabilities/plan/service/service.go#L222-L257)<br>[todo_write.go:L82-93](file:///d:/workspace/go/genesis-agent/internal/capabilities/tool/adapter/builtin/todo_write.go#L82-L93) | `Implemented` | 对齐。重大重构自动打标 `StepStatusBlockedByApproval`，并在 `todo_write` 执行返回中提示系统弹起审批流。 |
| **数据结构树形化支持 (ParentID)** | § 5.1 | [plan.go:L28](file:///d:/workspace/go/genesis-agent/internal/capabilities/plan/model/plan.go#L28)<br>[service.go:L182-220](file:///d:/workspace/go/genesis-agent/internal/capabilities/plan/service/service.go#L182-L220) | `Implemented` | 对齐。`Step` 增加 `ParentID`，同时在全量更新计划时进行循环依赖检测。 |
| **事件广播契约** | § 5.2 | [broadcaster.go](file:///d:/workspace/go/genesis-agent/internal/capabilities/plan/contract/broadcaster.go) | `Implemented` | 对齐。接口与负载结构完整定义并在 PlanService 中触发广播。 |
| **本地存储适配器** | § 7.1 | [file_repo.go](file:///d:/workspace/go/genesis-agent/shared/local/plan/file_repo.go) | `Implemented` | 对齐。实现并发读写安全（`sync.RWMutex`），支持主快照与审计日志追加（`SavePlan`），且已补全基于正则的 SessionID 安全过滤。 |
| **CLI TUI 渲染** | § 9.1 | [plan_renderer.go](file:///d:/workspace/go/genesis-agent/products/cli/internal/tui/plan_renderer.go) | `Implemented` | 对齐。使用 `lipgloss` 样式以及 Unicode 连接符呈现了淡翠绿/极光青等高质感 TUI。 |
| **企业 SaaS 多租户 DB 路由** | § 7.2 | - | `Unimplemented (Phase 2 暂缓)` | 对齐设计决策。由于 Phase 1 环境没有引入多租户 DB，按规划暂缓实施。 |
| **网页端玻璃态卡片与前端微动效** | § 9.2 | - | `Unimplemented (Phase 2 暂缓)` | 对齐设计决策。与 Desktop/Enterprise 前端初始化同步暂缓。 |

### 12.2 Gaps & 反思说明

- **系统提示词约束力**：提示词规范在设计文档中已经完整定义，且被动提醒功能通过 `ContextInjectorFunc` 挂载，但 Agent 是否始终严格遵循“更新后免背诵/免 Markdown 手动复述列表”约束，需依靠基座模型本身对 System Instruct 的遵循度。若后续观察到复述问题，需要在 prompt 注入器中进一步强制收紧提示词结构。

### 12.3 下一步行动计划 (Actionable Next Steps)

- **[ ] 挂载 Enterprise 真实的 EventBroadcaster 与 REST 接口**
  - **位置**：`products/enterprise/...`
  - **内容**：在 Enterprise 启动时将 `broadcaster` 契约替换为实际的 SSE 或 MQ 广播实现。
