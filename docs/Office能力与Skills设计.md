...
- 多个 Skills：`office-word`、`office-excel`、`office-ppt`、`office-pdf`。
...
Agent Runtime
  -> Skill System
     -> office-word / office-excel / office-ppt / office-pdf
  -> Tool Gateway
...
| `office-pdf` | PDF 读写、合并拆分、创建、填表、OCR、预览 | `.pdf`、表单、扫描件 | `.pdf`、抽取结果、页图、填表结果 |
...
office-word/
  ...
  scripts/
    ...
    accept_changes.py
    templates/
    office/          # pack/unpack/validate/soffice/helpers/validators/schemas
...
| PDF | 覆盖 pypdf、pdfplumber、reportlab、qpdf、pdftk、Poppler、OCR、表单脚本 | **已按** `docs/office-pdf技能迁移设计.md` 整删旧 `pdf-review` 后 verbatim 迁入 Anthropic `pdf` → `office-pdf`（FORMS/REFERENCE + 表单脚本；frontmatter + bridge） | 与 office-ppt/word 同范式；宿主适配下沉运行时 |
| DOCX | 深度使用 docx-js、pandoc、OpenXML 解包/打包、comments、tracked changes、schema validator | **已按** `docs/office-word-docx技能迁移设计.md` 整删旧包后 verbatim 迁入 Anthropic `docx` → `office-word`（自包含 `scripts/office`、`comment.py`、`accept_changes.py`；frontmatter + Dependencies 最小适配） | 与 office-ppt 同范式：技能内容保持 Anthropic；执行走 `run_skill_command` + bridge，不在技能内写 Genesis 硬约束 |
| PPTX | 强调视觉设计、模板编辑、缩略图、LibreOffice+Poppler 渲染 QA、pptxgenjs | 已按 verbatim 范式迁入 `office-ppt`（`pptxgenjs` + thumbnail/editing；`dependencies.runtime`；见第三方 Skill 执行模型文档） | 优先 Anthropic 成熟流程；宿主适配下沉运行时 |
| XLSX | 强调公式而非硬编码、LibreOffice 重算、公式错误 JSON、金融模型格式规范 | **已按** `docs/office-excel-xlsx技能迁移设计.md` 整删旧包后 verbatim 迁入 Anthropic `xlsx` → `office-excel`（`recalc.py` + openpyxl/pandas；frontmatter + bridge） | 与 office-ppt/word/pdf 同范式；Excel 主 QA 为 `recalc.py`（上游 validate 不支持 xlsx） |
| 共享 office 脚本 | pack/unpack/validate/soffice helpers 和大量 schema | **各 Skill 自包含** `scripts/office/**`（与 Anthropic 一致；废止 `_office_common`） | drop-in 可移植优先于仓库内 DRY |
| OCR | PDF 中强调扫描件 OCR | 扩展为内容/操作触发，Word/PPT/Excel 内嵌图片也可升级 `office-ocr` | 更符合真实 Office 文档，避免把 OCR 错误绑定到扩展名 |

结论：Anthropic 的成熟设计证明了“脚本 + 验证闭环 + Office 专用 runtime”是正确方向。`office-ppt` / `office-word` / `office-pdf` / `office-excel` 均已按 verbatim 迁入。许可为 Proprietary 时须确认再分发权利。
...
- 新增 `office-word`、`office-excel`、`office-ppt`、`office-pdf` 四个 Skill。
...
| 内置 `office-word`、`office-excel`、`office-ppt`、`office-pdf` 四个 Skill | 第五章、第六章、第十一章 Phase 1 | `internal/capabilities/skill/adapter/embedded/skills` 已包含四个 Office Skill；embedded source 单测覆盖发现和资源读取 | `[x] 已实现` | 四个 Skill 均已 Anthropic verbatim（excel 见 `docs/office-excel-xlsx技能迁移设计.md`）。 |
| Office 脚本库：docx/xlsx/pptx/pdf 检查、转换、渲染 | 第六章、第七章、第十一章 Phase 2 | `office-word`/`office-ppt`/`office-pdf`/`office-excel` 已 verbatim 迁入 Anthropic 脚本 | `[x] 已实现` | Excel 主路径为 openpyxl/pandas + `recalc.py`；端到端样例与产品化冒烟可继续加强。 |
...
- 在 `internal/capabilities/skill/adapter/embedded/skills` 下新增 `office-word`、`office-excel`、`office-ppt`、`office-pdf`。
...
- `office-word`：已按 `docs/office-word-docx技能迁移设计.md` 整删旧 inspect/path_contract，迁入 Anthropic `docx`（docx-js 创建、comment/accept_changes、office validate/unpack/pack）。
- `office-excel`：已按 `docs/office-excel-xlsx技能迁移设计.md` 整删旧 inspect/`recalc_xlsx`/`path_contract`，迁入 Anthropic `xlsx`（openpyxl/pandas、`scripts/recalc.py`、自包含 `scripts/office`）。
- `office-ppt`：已 verbatim 迁入 Anthropic pptx 脚本（thumbnail/editing/pptxgenjs 等）。
- `office-pdf`：已按 `docs/office-pdf技能迁移设计.md` 整删旧 `pdf-review`，迁入 Anthropic `pdf`（reportlab 创建、FORMS 填表脚本、REFERENCE、Poppler/qpdf/OCR 路径）。
- ~~`office-word`：`convert_docx_to_pdf.py`~~ → 使用 `scripts/office/soffice.py --convert-to pdf`（Anthropic 原文）。
- `office-ppt`：预览走 thumbnail + soffice/pdftoppm。
- ~~`office-excel`：`recalc_xlsx.py`~~ → 使用 `scripts/recalc.py`（Anthropic 原文）。
