...
- **Tool 负责入口**：提供 `load_skill`、文件读写、命令执行等少量稳定原语。
...
一句话：Office 是领域工作流，不是工具清单。Skill 是主要承载形式，Tool 只是通用执行入口，Plugin 是安装和分发边界。
...
Agent Runtime
  ...
  -> Tool Gateway
     -> load_skill / read_file / write_file / edit_file / run_command
  -> Execution Runtime
...
| 层级 | 负责 | 不负责 |
| --- | --- | --- |
| Skill | Office 任务流程、约束、格式规范、验证步骤 | 直接绕过权限执行文件和网络操作 |
...
| PPTX | 强调视觉设计、模板编辑、缩略图、LibreOffice+Poppler 渲染 QA、pptxgenjs | 已提供 inspect 和 PPTX 预览渲染脚本，保留“必须视觉 QA”的流程 | 通用模型更需要短流程和强验证；设计长指南后续可拆 reference，不塞满主 Skill |
...
- 复用现有 `load_skill`、`read_skill_resource`、文件工具和 `run_command`。
- 明确依赖声明和审批策略。
...
- Skill 工具：`internal/capabilities/skill/tool/load_skill`、`read_skill_resource`、`search_skill_resources`
...
| --- | --- | --- | --- | --- |
| Office 能力以 Skill 承载流程，而不是注册大量细粒度 Office Tool | 第一章、第四章、第五章 | 当前没有 `word.create_document`、`excel.set_cell` 等 Office 细粒度工具；工具层主要是 `read_file`、`write_file`、`edit_file`、`run_command`、`load_skill` 等通用入口 | `[x] 已对齐` | 代码没有走反方向，后续应继续避免把 Office 领域动作硬编码成大量 Tool。 |
...
| `load_skill`、`read_skill_resource`、`search_skill_resources` 可作为 Skill 入口 | 第一章、第四章、第十一章 | CLI bootstrap 注册 `load_skill`、`read_skill_resource`、`search_skill_resources`；Skill service 能渲染可用技能列表并注入 Prompt | `[x] 已实现` | 目前主要在 CLI 装配完整；Enterprise/Debug API 尚未统一暴露 Skill 管理和运行入口。 |
...
| 验证与测试 | 第六章、第十一章 | Skill parser、service、load_skill、run_command、sandbox runner、skillmarket、HTTP sandbox adapter 和 embedded Office Skill 发现均有单测 | `[/] 部分实现` | 还需要 Office 样例文件、脚本单测、可打开性校验和 CLI 集成 smoke test。 |
...
- `load_skill` 能检查依赖，但依赖声明还不是 Execution policy 的强约束来源；Office 所需的 `python`、`libreoffice`、远程沙箱、MCP connector 仍需要明确的策略映射。
...
- `genesis-cli` 的 Skills 注入列表能出现四个 Office Skill。
- `load_skill` 能加载四个 Skill，并返回依赖报告。
