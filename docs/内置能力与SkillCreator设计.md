...
| 类型        | 示例                                     | 放置建议                                                                |
| --------- | -------------------------------------- | ------------------------------------------------------------------- |
| 内置 Tool   | `read_file`、`run_command`、`Skill`（） | `internal/capabilities/*/tool`；协议见 `docs/superpowers/specs/2026-07-09-skill-tool-protocol-boundary-design.md` |
...
- `allowed-tools` 需要映射到 Genesis 工具名：`read_file`、`write_file`、`edit_file`、`run_command`、`Skill`（及 skill 资源工具）。空声明不收窄；非空与 Profile 求交。
...
- 调用 `Skill(skill="skill-creator")`后能读取 Genesis 版 `skill-creator` 的完整说明和引用资源。
...
| 外部 Skill 安装不自动扩权                                                 | 第 3.2、6.3、11.2       | 安装记录只保存 package/source/scope/provenance；执行仍通过 `Skill` 网关、Tool/Approval/Policy/Sandbox 流程                                                                         | `/` 部分实现 | 没有发现安装即授予 Tool 权限的实现；协议边界 Phase 1 将统一网关名与 CollisionGuard。                                   |
