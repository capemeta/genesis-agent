...
**Skill / Tool 协议边界**（网关名 `Skill`、CollisionGuard、Catalog 主通道、分期落地）以专项设计为准：

- `docs/superpowers/specs/2026-07-09-skill-tool-protocol-boundary-design.md`

**Skill 调用原理对照**（Kode / Codex / Genesis：提示词如何引导、模型如何返回、返回后如何执行）：

- `docs/Skill调用原理对比-Kode-Codex-Genesis.md`

**Skill 触发模式**（用户显式 `/`·`$` / 意图匹配 / Agent 自判；缺口与实现顺序）：

- `docs/superpowers/specs/2026-07-11-skill-trigger-modes-design.md`

**Skill 远程安装**（GitHub URL / 可配置域名 / CLI 与对话工具；三产品边界）：

- `docs/superpowers/specs/2026-07-14-skill-remote-install-design.md`

本文其余章节若与上述专项冲突，以专项为准。本文 §3.4 / §3.6 / §6.5 收录远程安装的配置与命令行摘要，便于查阅。
...
| Skill Tool         | LLM 通过 `Skill` 工具显式加载技能                                                                          | **对齐 Kode**：LLM 可见工具名固定为 `Skill`（参数 `skill`/`args`）；必须走 ToolGateway/Approval；skill 名永不进 function schema。协议细节以 `docs/superpowers/specs/2026-07-09-skill-tool-protocol-boundary-design.md` 为准 |
...
internal/capabilities/skill/
  ...
  tool/
    skill/                 # 实现包；LLM 对外工具名固定为 Skill
    list_skill_resources/ # 列出 skill 包内资源
    read_skill_resource/  # 读取 skill 包内资源
    search_skill_resources/ # 搜索 skill 包内资源
  collision/              # skill 名误当 tool 名的匹配与纠错结果（Gateway 不依赖本包）
  marketplace/
...
| `dependencies`              | 否   | 分层：`tools`（MCP/tool/connection，加载前校验与审批）；`runtime`（python/node/system 包，供 preflight 与 `install_skill_dependencies`）。详见 `docs/Skill三模式执行与依赖闭环设计.md`                                                                                |
| `requires`                  | 否   | 能力门槛列表。每项含 `kind`（如 `vision`）与 `enforcement`。**仅** `enforcement: required` **会在** `Skill` **加载时硬拒绝**；省略或 `optional` 与不配置等价，不做限制。`vision` 是否可用看本 Run 的 EffectiveVisionMode（主模型或 `router.vision` 的真实 `supports_image=true`），不是模型名猜测 |
| `qa`                        | 否   | 交付侧质量策略：`policy`（如 `visual-qa/v1`）+ `enforcement`。**仅** `required` **阻塞 Run 完成**；`optional`/省略与不配置等价，不因缺 QA 失败。有视觉时仍可做视觉检查并记证据，属于加分而非门禁                                                                                           |




#### `requires` / `qa.enforcement` 用法

```yaml
# 以看图为交付前提的技能：没视觉模型就不该启动
requires:
  - kind: vision
    enforcement: required

# PPT 等：视觉 QA 有帮助，但不是交付前提（与不写 enforcement 相同）
qa:
  policy: visual-qa/v1
...
1. **默认放行**：不写 `requires` / `qa.enforcement`，或写 `optional`，系统不做能力/QA 硬限制。
2. **硬限制唯一开关**：只有 `enforcement: required`。
3. **视觉真相源**：`configs/llm.yaml` 里实际会用的模型（默认主模型或 `router.vision`）的 `supports_image: true`；二者皆无则 EffectiveVisionMode=`degraded_text`，`requires.vision=required` 的技能加载失败。
4. **父子交付**：fork 子 Agent 已完成交付后，父侧靠 Adoption 销账，不必再 view 子 QA 图、也不必再 `select` 子产物。
...
| 输入                  | 示例                                                      | 说明                                                                |
| ------------------- | ------------------------------------------------------- | ----------------------------------------------------------------- |
| GitHub 目录 URL       | `https://github.com/org/repo/tree/main/skills/foo`      | **推荐用户粘贴形态**；解析 owner/repo/ref/sub_path，zip 下载                    |
| GitHub blob URL     | `https://github.com/.../blob/main/skills/foo/SKILL.md`  | 归一到 skill 目录                                                      |
| GitHub shorthand    | `owner/repo[@ref][#path]`                               | 默认下载 zip，不要求本机 git；host 固定为 `github.com`                          |
| GitHub explicit     | `github:owner/repo@v1.0.0#skills/foo`                   | 推荐用于可审计安装                                                         |
| 兼容主机 URL            | `https://git.example.com/org/repo/tree/main/skills/foo` | 须 `/tree` 或 `/blob`；主机在 `allowed_hosts`                           |
| 下载站 / 裸包 URL        | `https://openskills.cc/api/download?slug=...`           | 走 URL 通道（看 Content-Type/魔数）；主机须在 `allowed_hosts`，无需 `--allow-url` |
| Git URL             | `git:https://host/org/repo.git@ref#path`                | 可转换为兼容主机时按 github 语义拉取                                            |
| URL 显式              | `url:https://host/marketplace.zip` 或 `.json`            | 白名单外域名需 `--allow-url` 或对话 Approval；Enterprise 默认禁用                |
| Local directory     | `dir:D:\skills\foo`                                     | CLI/Desktop；受 `allow_local` 控制                                    |
| Local file          | `file:D:\skills\marketplace.json`                       | 只作为 manifest                                                      |
| package@marketplace | `document-skills@office-skills`                         | 从已登记 marketplace 安装包                                              |
| Enterprise catalog  | `enterprise:<catalog>/<package>`                        | 只由 Enterprise Source/DB 实现                                        |




#### 3.4.1 远程安装流水线（InstallFromSource）

用户说「下载并安装这个技能 https://…」时，走统一编排，而不是 Agent 用 `run_command` / curl / git clone 旁路：

```text
SourceParser（internal，含 tree/blob）
  -> AllowedSourcePolicy（allowed_hosts）
  -> Fetcher（CLI/Desktop: shared/local/skillmarket）
  -> PackageDetector（marketplace / 单 Skill 合成 / 多 Skill 候选）
  -> Installer + InstallStore + SourceProvenance
  -> CatalogReloader（可选；未注入则下一回合生效）
```

要点：

- **单 Skill 目录**（仅有 `SKILL.md`、无 marketplace.json）会合成 ephemeral `skill-package`，进入既有 InstallRecord 管道。
- **多 Skill / 多 package** 时返回候选，禁止静默全装；CLI 用 `--skill-path` / 显式 `package@marketplace`。
- **对话工具**名为 `install_skill_from_source`（装 Skill 本体）；与 `install_skill_dependencies`（装 npm/pip）严格区分。
- 审批动作为 `ActionSkillInstall`（高风险）；禁止复用 `ActionCommandExec`。
- 安装 ≠ 授权执行；装完后仍须 `Skill(skill=...)` 加载，且 `allowed-tools` / Profile 求交不变。
- Enterprise：默认拒绝远程；走 Import → pending → published（详见专项设计 P2）。



#### 3.4.2 配置：`skills.install`

配置写在 `configs/config.yaml`、`configs/config.local.yaml` 或 `~/.genesis-agent/cli/config.yaml`：

```yaml
skills:
  install:
    # 允许的远程主机（forge + 下载站共用同一白名单）。
    # - forge：仅识别恰好 owner/repo，或 /tree|/blob/<ref>/...（推荐粘贴 tree 链接）
    # - 下载站：带 query 或非 tree/blob 路径 → SourceTypeURL，按 zip/json 内容拉取
    # 空或省略时默认仅 github.com。
    allowed_hosts:
      - github.com
      # - openskills.cc            # 技能下载站（API 返回 zip）
      # - github.mycompany.com     # 企业 GitHub Enterprise
      # - git.example.com          # 其它兼容 forge
    # 是否允许 dir:/file: 本地安装；省略默认 true（CLI/Desktop）
    allow_local: true
```
...
| 字段              | 类型         | 默认               | 说明                                       |
| --------------- | ---------- | ---------------- | ---------------------------------------- |
| `allowed_hosts` | `[]string` | `["github.com"]` | 白名单（forge 与下载站共用）；不在列表中会 `policy_denied` |
| `allow_local`   | `bool`     | `true`           | 是否允许 `dir:` / `file:` / 本地路径安装           |


行为说明：

- **Forge 解析**：带 query 的 URL、以及非 `/tree`/`/blob` 的深层路径，**不当** `owner/repo`（避免把 `/api/download` 误判为仓库）。
- `github.com` 仓库：走 `codeload.github.com` zip。
- 其它 forge 主机：按 GitHub 兼容语义解析；拉取尝试 `https://{host}/{owner}/{repo}/archive/{ref}.zip`（及 codeload 子域候选）。
- **下载站 / 裸 URL**（如 `openskills.cc/api/download?...`）：主机在白名单内即可安装；Fetcher 按 `Content-Type` / `Content-Disposition` / ZIP 魔数识别，不要求 URL 以 `.zip` 结尾。
- **重定向**：仅允许同主机（含端口）跳转，或目标 Hostname 在 `allowed_hosts` 中；`github.com` 额外放行 `codeload.github.com` / `raw.githubusercontent.com` 等官方下载子域；其它 forge 放行 `codeload.<host>`。
- shorthand `owner/repo` 始终视为 `github.com`。
- 白名单外的任意公网 URL 仍需 `--allow-url`（或对话 Approval 覆盖）。
- 对话路径即使用户给出 URL，仍须 Approval；配置白名单不能代替审批。
- Enterprise 默认另用 deny-all 策略桩，不直接读本配置做公网安装（管理面 Import 另议）。

安全规则：

- 所有 manifest 中的 `source`、`skills`、`commands`、`assets` 必须是 `./...` 相对路径，禁止绝对路径、反斜杠、`..`、软链接逃逸。
- 安装前必须读取并校验每个 `SKILL.md` frontmatter，`name` 必须与目录一致（安装落盘后的目录名），description 必填且不超过 1024 字符。
- GitHub/URL 下载必须进入临时目录，验证通过后原子替换 cache/install 目录；失败要清理临时目录。
- 安装记录必须保存 `SourceProvenance`（type/address/domain/repo/ref/sub_path/hash 等）以及 `installed_at/enabled/scope`。
...
# 从已登记 marketplace 安装包（兼容）：
genesis skill install <package>@<marketplace> [--scope user|project] [--force]

# 从 GitHub / 兼容主机 URL / 本地源安装（推荐用户粘贴地址时用）：
genesis skill install <url-or-source> [--scope user|project] [--force] [--skill-path <rel>] [--allow-url]
...
```powershell
# 公开 GitHub skill 目录
genesis skill install "https://github.com/org/repo/tree/main/skills/foo"

# shorthand / 显式前缀
genesis skill install "org/repo@main#skills/foo"
genesis skill install "github:org/repo@v1#skills/foo"

# 企业兼容主机（须先写入 skills.install.allowed_hosts）
genesis skill install "https://git.example.com/org/repo/tree/main/skills/foo"

# 多 Skill 仓库指定路径
genesis skill install "https://github.com/org/repo/tree/main/skills" --skill-path "office-ppt"

# 本地目录（allow_local=true）
genesis skill install "dir:D:\workspace\skills\my-skill"

# 非 GitHub 的 .zip URL（须显式放行）
genesis skill install "https://cdn.example.com/skills/pack.zip" --allow-url

# 项目级安装
genesis skill install "https://github.com/org/repo/tree/main/skills/foo" --scope project --force
```

对话期：用户给出 Skill 地址时，Agent 应调用工具 `install_skill_from_source`（须审批），**禁止**用 `run_command` / curl / git clone 旁路。装依赖包仍用 `install_skill_dependencies`。
...
| 参数                     | 说明                                             |
| ---------------------- | ---------------------------------------------- |
| `--scope user|project` | 默认 `user`；project 写入当前 workspace `.genesis`    |
| `--force`              | 覆盖已安装同名 package                                |
| `--skill-path`         | 多 Skill 来源时指定相对目录                              |
| `--allow-url`          | 允许**不在** `allowed_hosts` 中的公网 URL；白名单内下载站无需此开关 |


命令行为：

- `list/search/show` 只读目录，不安装；默认可显示 marketplace load errors 的摘要，`--json` 输出完整错误数组。
- `marketplace add/update/remove` 会触发 Approval；URL/GitHub/Git 来源必须经过 AllowedSourcePolicy（`skills.install.allowed_hosts`）。
- `install <url-or-source>` 走 `InstallFromSource`：解析 → 策略 → 拉取 → Detect → 落盘；多候选时非 0 退出并打印候选列表。
- `install <package>@<marketplace>` 委托既有 marketplace 包安装。
- `installed` 只读取 InstallStore，不强制刷新远程目录，避免网络失败影响本地状态查询。
- `enable/disable/uninstall` 修改安装状态后，CLI bootstrap 下一次构建 SkillService 时只注入 enabled packages；当前会话若未注入 CatalogReloader，新装 Skill 可能需**下一回合**才进 Catalog。
...
- Codex 的 `skill-installer`（GitHub URL 装单 Skill）能力已吸收为 `InstallFromSource` + CLI/对话双入口，不以 Python 脚本为主路径。
...
products/<product>/bootstrap
  -> 构造 SkillSource 列表
  -> 构造 SkillService
  -> 注册 Skill 网关工具及 read/list/search_skill_resources
  -> 注入 CollisionGuard / UnresolvedNameHandler（可选）
  -> Skill 工具 DescriptionFunc 绑定 Catalog（主通道）
  -> system prompt 仅注入短策略硬规则（不再重复完整 skills 列表）
```
...
### 4.4 Prompt 与 Catalog 注入

**协议边界（以** `docs/superpowers/specs/2026-07-09-skill-tool-protocol-boundary-design.md` **为准）：**

- Catalog（`<available_skills>`）主通道挂在 `Skill` **工具的动态 description**（`DescriptionFunc`），随每轮 `ListInfos`/LLM 绑定刷新。
- System prompt 只保留 3–5 行硬规则：禁止把 skill 名当 tool 名；必须先调用 `Skill(skill=...)` 再使用原语工具。
- Progressive disclosure：初始上下文不放 skill body；body 仅在 `Skill` 加载后以单份 `<skill_injection>` 注入。
- `location` 只暴露 opaque locator，禁止宿主机绝对路径。

预算：

- available skills fragment 默认 8KB 字符，可与约 2% context token 上限取更严者；超出截断并标注 `Showing X of Y`。
- 单个 `SKILL.md` 主体默认 8KB，超出返回 truncated。
- description 最大 1024 字符。
- catalog 按 scope 优先级 + name 排序。

`$skill` / mention / `SelectForTurn` 已接线（协议边界 Phase 2），**不替代** `Skill` 网关；行为缺口与 slash UX 见触发模式设计。

### 4.5 Skill 网关工具

**LLM 可见工具名：**`Skill`**（对齐 Kode）。**  
原语工具继续 snake_case；元工具允许 `Skill` 专名。不保留 `load_skill` 别名或旧 `name` 参数；schema / 日志 / Profile / 提示词只认 `Skill(skill=...)`。
...
兼容：旧字段 `name` 在解析层映射为 `skill`。

行为：

- 先走 ToolGateway / Approval。
- `Resolve(ModelCall=true)` → 依赖预检 → `Load`。
- ToolResult 只返回短确认元数据（不含完整 body）。
- Runtime 追加独立 `<skill_injection>` 承载 body（单份，避免 ToolResult 双份正文）。
- `allowed-tools`：空=不收窄；非空与当前可见集求交后经 `Gateway.FilterInfos`；求交为空则失败。
- 同轮若同时出现其他 tool call：只执行 `Skill`，其余跳过（独占轮）。



### 4.6 Skill 名误当 Tool 名（CollisionGuard）

当模型调用 `office-ppt` 这类 **Catalog 中的 skill 名** 作为 tool name 时：

- 不得仅返回“未被当前产品 Profile 允许”（假原因）。
- 应返回结构化 `skill_tool_collision`，提示改用 `Skill(skill="...")`。
- Guard 放在 React Loop 执行前钩子，或 Gateway 可选 `UnresolvedNameHandler`（由 bootstrap 注入）；**禁止** `tool/gateway` 直接依赖 `skill` 实现。
- 同名 Tool 与 Skill 并存时 Tool 优先；安装/启动应对冲突打 warning。
- `auto_rewrite_skill_collision` 默认关闭。
...
- 支持 `genesis skill marketplace add <source>`、`genesis skill install <url-or-source|package@marketplace> --scope user|project`、`enable/disable/uninstall/list`。
- 远程安装主机白名单见配置 `skills.install.allowed_hosts`（§3.4.2）；对话安装工具 `install_skill_from_source`。
- 第三方 marketplace cache 放用户配置目录，例如 `%USERPROFILE%\.genesis-agent\cli\marketplaces\`；安装记录放 `%USERPROFILE%\.genesis-agent\cli\installed-packages.json` 或等价文件。
- project scope 安装只写 `<workspace>\.genesis\installed\packages` 等项目目录，并记录 `project_path`，避免跨项目误用。
...
`allowed-tools` **不是扩权白名单**，而是激活后的工具可见上限：


| 声明              | 行为                                                   |
| --------------- | ---------------------------------------------------- |
| 空 / 未声明         | 不收窄，保持当前 turn 可见工具集                                  |
| 非空              | 与当前可见集及 Profile 求交；经 `Gateway.FilterInfos`；求交为空则加载失败 |
| 扩权到 Profile 外工具 | **禁止**静默扩权；不得因 skill 声明而 Register 新工具                |
...
Product Profile
  -> ToolGateway
  -> Approval / RBAC
  -> RuntimeContext.AllowedTools（skill 激活后可选收窄）
  -> ToolTraits / ResourceLock / Audit
```

Skill 名误当 Tool 名时走 CollisionGuard（§4.6），不走“Profile 未允许”假语义。
...
```text
MarketplaceSource
  -> AllowedSourcePolicy（skills.install.allowed_hosts）
  -> Download/Import Adapter
  -> Manifest Validator / PackageDetector
  -> Package Installer
  -> InstallStore
  -> SkillSource
```

AllowedSourcePolicy 当前落地与建议扩展：


| 策略                     | 示例 / 配置                                                       | 说明                           | 状态               |
| ---------------------- | ------------------------------------------------------------- | ---------------------------- | ---------------- |
| allow git hosts        | `skills.install.allowed_hosts: [github.com, git.example.com]` | CLI/Desktop 默认仅 `github.com` | **已落地**          |
| allow local            | `skills.install.allow_local: true`                            | 允许 `dir:`/`file:`            | **已落地**          |
| allow github repo      | `github:owner/repo`                                           | 只允许指定仓库                      | 建议扩展             |
| deny url               | 默认拒绝非白名单域名；裸 zip 需 `--allow-url`                              | 防任意公网                        | **已落地（域名级）**     |
| require ref            | `require_ref: true`                                           | 禁止安装 floating main/master    | 建议扩展             |
| require hash/signature | `require_signature: true`                                     | 企业发布必选                       | 建议扩展（Enterprise） |


`MarketplaceSource` 不等于可信来源。即使来源被允许，具体 Skill 在加载和执行时仍必须经过 `SkillPolicy`、`Approval`、`ToolGateway`、`RBAC` 和 `Audit`。

配置与命令行细节见 §3.4.2、§3.6；完整分期与三产品边界见 `docs/superpowers/specs/2026-07-14-skill-remote-install-design.md`。
...
审批与 `policy.permission_mode` 对齐（详见 `docs/统一配置权限与审批治理设计.md` §4.0）：


| 动作                             | `agent`    | `workspace_auto` | `full_access` |
| ------------------------------ | ---------- | ---------------- | ------------- |
| `skill.load` / 资源读             | Allow（可信包） | Allow            | Allow         |
| skill-fork `subagent.delegate` | Ask        | Allow            | Allow         |
| `run_skill_command`            | Ask        | Allow            | Allow         |
| `install_skill_from_source`    | Ask        | **Deny**         | Allow（仍建议审计）  |
...
6. `service.RenderAvailableSkills`：bounded catalog XML（供 `Skill` DescriptionFunc）。
7. runtime：`DescriptionFunc`、CollisionGuard、单份 injection、`FilterInfos` 收窄。
8. `Skill` 网关工具（实现包 `tool/skill`）：只调 SkillService，不直接读来源。
9. CLI bootstrap 注入 source；Profile 启用 `Skill`、`list_skill_resources`、`read_skill_resource`、`search_skill_resources`。
10. tests：parser、source、service、冲突解析、prompt budget、collision、`Skill` 加载。
11. 协议边界完整设计与分期：`docs/superpowers/specs/2026-07-09-skill-tool-protocol-boundary-design.md`。
...
| LLM 可见工具名                   | 固定为 `Skill`（对齐 Kode）；skill 名永不进 function schema                                                                                 |
| Catalog 放哪里                 | 主通道：`Skill` 动态 description；system 仅短硬规则                                                                                         |
| Skill 名误当 Tool 名            | CollisionGuard → `skill_tool_collision`；禁止仅返回 Profile 未允许                                                                       |
...
- **被审计代码 (Implementation Inspected)**:
  ...
  - 工具集: [skill/tool.go](file:///d:/workspace/go/genesis-agent/internal/capabilities/skill/tool/skill/tool.go)（对外名 `Skill`）, [read_skill_resource/tool.go](file:///d:/workspace/go/genesis-agent/internal/capabilities/skill/tool/read_skill_resource/tool.go), [search_skill_resources/tool.go](file:///d:/workspace/go/genesis-agent/internal/capabilities/skill/tool/search_skill_resources/tool.go)
  - 协议边界设计: [2026-07-09-skill-tool-protocol-boundary-design.md](file:///d:/workspace/go/genesis-agent/docs/superpowers/specs/2026-07-09-skill-tool-protocol-boundary-design.md)
...
| **3.3 Skill 格式 (YAML frontmatter)**               | 三、Genesis 目标架构        | [markdown.go](file:///d:/workspace/go/genesis-agent/internal/capabilities/skill/parser/markdown.go)                                                                                                                          | `Implemented`   | 支持 `allowed-tools`、`context`、`dependencies`、`requires`、`qa` 等字段的结构化解析；`enforcement` 仅 `required` 硬限制。            |
...
| **6.1-6.4 权限、依赖与安全**                              | 六、权限、依赖与安全            | [skill/tool.go](file:///d:/workspace/go/genesis-agent/internal/capabilities/skill/tool/skill/tool.go), [source.go](file:///d:/workspace/go/genesis-agent/shared/local/skill/source.go)                                       | `Implemented`   | 采用 `isWithin` 防止文件路径越界越权；使用通用 `Approval` 机制对高风险依赖进行授权拦截。                                                         |
