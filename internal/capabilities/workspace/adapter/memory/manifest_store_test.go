...
func TestManifestStoreRejectsCrossTenantLookup(t *testing.T) {
	...
	manifest := workmodel.RunManifest{SchemaVersion: workmodel.RunManifestSchemaVersion, Revision: 1, RunID: "run", Scope: scope, AgentApp: agentappmodel.EffectiveProfile{ID: "app", Version: "1", Workspace: agentappmodel.WorkspaceSpec{DefaultMode: execmodel.WorkspaceModeProject, AllowedModes: []execmodel.WorkspaceMode{execmodel.WorkspaceModeProject}, DefaultAccess: execmodel.WorkspaceAccessReadWrite}}, StateRoot: workmodel.StateRoot{ID: "state", Authority: "test", Scope: scope}, Limits: workmodel.WorkspaceLimits{MaximumAccess: execmodel.WorkspaceAccessReadWrite}, Executions: []workmodel.PreparedExecutionSnapshot{{Binding: binding, Backend: testBackend(), Workspace: execmodel.ExecutionWorkspace{WorkDir: "/project"}}}, Inputs: workmodel.InputManifest{RunID: "run", BindingID: "binding"}, View: workmodel.WorkspaceViewManifest{BindingID: "binding", Root: "."}, CreatedAt: time.Now().UTC()}
	if err := store.Create(context.Background(), manifest); err != nil {
		t.Fatal(err)
	}
