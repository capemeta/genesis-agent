// Package memory 提供 PlanRepository 的进程内实现。
package memory
...
func New() *Repository {
	return &Repository{plans: map[string]*model.Plan{}, history: map[string][]model.RevisionLog{}}
}

func (r *Repository) GetPlan(ctx context.Context, sessionID string) (*model.Plan, error) {
...
func (r *Repository) SavePlan(ctx context.Context, plan *model.Plan, revision *model.RevisionLog) error {
	if err := ctx.Err(); err != nil {
		return err
	}
...
func valueSessionID(plan *model.Plan) string {
	if plan == nil {
		return ""
	}
