...
func TestStageInputsResolvesFromOutputDir(t *testing.T) {
	...
	staged, err := stageInputs(root, ws, []string{"deck.pptx"})
	if err != nil {
		t.Fatal(err)
	}
	...
	dest := filepath.Join(ws.InputDir, "deck.pptx")
...
func TestStageInputsResolvesLogicalOutputPrefix(t *testing.T) {
	...
	staged, err := stageInputs(root, ws, []string{"$OUTPUT_DIR/out.pptx"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestStageInputsResolvesLogicalWorkPrefix(t *testing.T) {
	...
	staged, err := stageInputs(root, ws, []string{"$WORK_DIR/deck_gen.js"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestStageInputsStillRejectsOutsideWorkspace(t *testing.T) {
	...
	_, err := stageInputs(root, ws, []string{"../outside.pptx"})
