...
func (m *MockRepository) GetTaskList(ctx context.Context, sessionID string) (*model.TaskList, error) {
	return m.plan, nil
}

func (m *MockRepository) SaveTaskList(ctx context.Context, plan *model.TaskList, revision *model.RevisionLog) error {
	m.plan = plan
...
func TestTaskListService_SingleInProgressConstraint(t *testing.T) {
	repo := &MockRepository{}
	svc := NewTaskListService(repo, nil, 3)
	...
	_, err := svc.UpdateTaskList(context.Background(), "session_1", steps, "Test reason", "agent")
	if err == nil {
...
func TestTaskListService_LevenshteinAndIDAlignment(t *testing.T) {
	repo := &MockRepository{}
	svc := NewTaskListService(repo, nil, 3)
	...
	plan1, err := svc.UpdateTaskList(context.Background(), "session_1", steps1, "Init plan", "agent")
	if err != nil {
	...
	plan2, err := svc.UpdateTaskList(context.Background(), "session_1", steps2, "Tweak title", "agent")
	if err != nil {
...
func TestTaskListService_MergeCollaborativePlan(t *testing.T) {
	repo := &MockRepository{}
	svc := NewTaskListService(repo, nil, 3)
	...
	plan1, _ := svc.UpdateTaskList(context.Background(), "session_1", steps, "Initial", "agent")
	...
	plan2, err := svc.UpdateTaskList(context.Background(), "session_1", stepsAI, "AI update", "agent")
	if err != nil {
...
func TestTaskListService_StructuralRePlanningInterception(t *testing.T) {
	repo := &MockRepository{}
	svc := NewTaskListService(repo, nil, 3)
	...
	_, _ = svc.UpdateTaskList(context.Background(), "session_1", steps1, "Init", "agent")
	...
	plan2, err := svc.UpdateTaskList(context.Background(), "session_1", steps2, "AI replanning", "agent")
	if err != nil {
	...
	if !strings.HasPrefix(plan2.LatestExplanation, "[重大任务清单重构申请]") {
		t.Errorf("Unexpected explanation prefix: %s", plan2.LatestExplanation)
	}
}

func TestTaskListService_GeneratePromptReminder(t *testing.T) {
	repo := &MockRepository{}
	// remindIntervalSteps = 2
	svc := NewTaskListService(repo, nil, 2)

	// 1. 无清单：不每轮催促
	_, needed1, _ := svc.GeneratePromptReminder(context.Background(), "session_1", 0)
	if needed1 {
		t.Error("Expected needed to be false when no task list exists")
	}
	...
	_, _ = svc.UpdateTaskList(context.Background(), "session_1", steps, "Init", "user") // user 绕过审批阻断
	...
	if !strings.Contains(reminder3, "当前任务清单进度跟进") {
		t.Errorf("Unexpected reminder text: %s", reminder3)
	}
	if !strings.Contains(reminder3, "<system-reminder>") || !strings.Contains(reminder3, "</system-reminder>") {
		t.Errorf("reminder must be wrapped as system-reminder: %s", reminder3)
	}
}

func TestTaskListService_ParentIDValidationAndCycles(t *testing.T) {
	repo := &MockRepository{}
	svc := NewTaskListService(repo, nil, 3)
	...
	_, err := svc.UpdateTaskList(context.Background(), "session_1", selfRefSteps, "Self ref", "user")
	...
	_, err = svc.UpdateTaskList(context.Background(), "session_1", danglingSteps, "Dangling", "user")
	...
	_, err = svc.UpdateTaskList(context.Background(), "session_1", cycleSteps, "Cycle chain", "user")
	if err == nil || !strings.Contains(err.Error(), "存在父子循环依赖引用") {
		t.Errorf("Expected circular dependency error, got: %v", err)
	}
}

func TestTaskListService_DefensiveFallbacksAndValidations(t *testing.T) {
	repo := &MockRepository{}
	svc := NewTaskListService(repo, nil, 3)
	...
	plan, err := svc.UpdateTaskList(context.Background(), "session_1", steps, "", "agent")
	if err != nil {
