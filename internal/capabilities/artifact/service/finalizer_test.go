...
func (p *recordingPublisher) Publish(_ context.Context, req PublicationRequest) (artifactmodel.ArtifactRef, error) { p.calls=append(p.calls,req); return artifactmodel.ArtifactRef{ID:"artifact",Name:"deck.pptx"},nil }
...
func TestDeterministicFinalizerOnlyAutoSelectsUniqueMatch(t *testing.T) {
	ctx := context.Background(); ledger:=artifactmemory.NewStore(); produced:=workmemory.NewProducedResourceStore(); now:=time.Now().UTC()
	...
	if err:=ledger.CreateDeliverable(ctx,spec); err!=nil { t.Fatal(err) }
	...
	result,err:=finalizer.FinalizeRequired(ctx,"tenant","run"); if err!=nil { t.Fatal(err) }
	if len(result.Resolutions)!=1 || result.Resolutions[0].Status!="delivered" || result.Resolutions[0].SelectedID!="p1" { t.Fatalf("result=%+v",result) }
	selection,err:=ledger.GetSelection(ctx,"tenant","run","d1"); if err!=nil || selection.ProducedResourceID!="p1" { t.Fatalf("selection=%+v err=%v",selection,err) }
	if len(pub.calls)!=1 || len(delivery.calls)!=1 { t.Fatalf("publish=%d delivery=%d",len(pub.calls),len(delivery.calls)) }
}
...
func TestDeterministicFinalizerReturnsOpaqueIDsForMultipleMatches(t *testing.T) {
	...
	createProduced(t,produced,"p2","run:/work/b1/b.pptx","b.pptx","application/pptx",now); createProduced(t,produced,"p1","run:/work/b1/a.pptx","a.pptx","application/pptx",now)
	pub,delivery:=&recordingPublisher{},&recordingDelivery{}; finalizer,_:=NewDeterministicFinalizer(ledger,ledger,produced,pub,delivery)
	result,err:=finalizer.FinalizeRequired(ctx,"tenant","run"); if err!=nil { t.Fatal(err) }
	got:=result.Resolutions[0]; if got.Status!="selection_required" || len(got.CandidateIDs)!=2 || got.CandidateIDs[0]!="p1" || len(pub.calls)!=0 { t.Fatalf("result=%+v",result) }
	if _,err:=finalizer.SelectAndFinalize(ctx,"tenant","run","d1","p2"); err!=nil { t.Fatal(err) }
	if len(pub.calls)!=1 || pub.calls[0].ProducedResourceID!="p2" { t.Fatalf("publish=%+v",pub.calls) }
}
