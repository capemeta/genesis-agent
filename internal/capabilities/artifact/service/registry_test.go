...
func TestAdoptionStoreResolveAndVersionLock(t *testing.T) {
	store, err := NewAdoptionStore(t.TempDir())
	if err != nil {
		t.Fatal(err)
	}
	...
	// 2. 父子边界显式接纳。
	rec, err := store.Adopt(artifactcontract.AdoptionRecord{
	...
	if _, err := store.Adopt(artifactcontract.AdoptionRecord{ConsumerTenantID: consumerTenant, ConsumerRunID: consumerRun, ProducedID: producedID, OwnerTenantID: consumerTenant, OwnerRunID: ownerRun, ContentHash: "sha256:aaa"}); err != nil {
		t.Fatalf("idempotent adopt should not fail: %v", err)
	}

	// 5. 版本锁：同槽位接纳到不同内容哈希 → 冲突。
	if _, err := store.Adopt(artifactcontract.AdoptionRecord{ConsumerTenantID: consumerTenant, ConsumerRunID: consumerRun, ProducedID: producedID, OwnerTenantID: consumerTenant, OwnerRunID: ownerRun, ContentHash: "sha256:bbb"}); err == nil {
...
func TestAdoptionStoreDiskPersistence(t *testing.T) {
	stateRoot := t.TempDir()
	writer, err := NewAdoptionStore(stateRoot)
	if err != nil {
		t.Fatal(err)
	}
	if _, err := writer.Adopt(artifactcontract.AdoptionRecord{
		ConsumerTenantID: "t", ConsumerRunID: "run-parent", ProducedID: "produced-x",
		OwnerTenantID: "t", OwnerRunID: "run-child", ContentHash: "sha256:abc",
	}); err != nil {
		t.Fatalf("adopt failed: %v", err)
	}

	// 新实例（无内存缓存）应能通过传入 stateRoot 从磁盘恢复接纳记录。
	reader, err := NewAdoptionStore(stateRoot)
	if err != nil {
		t.Fatal(err)
	}
	got, ok := reader.Resolve("t", "run-parent", "produced-x")
